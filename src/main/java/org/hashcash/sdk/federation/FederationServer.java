...
import org.hashcash.sdk.requests.ResponseHandler;

/**
 * FederationServer handles a network connection to a
 * instance and exposes an interface for requests to that instance.
 *
 * For resolving a HC-Net address without knowing which federation server
...
public class FederationServer {
   ...
   * It tries to find a federation server URL in HC-Net.toml file.
  ...
  public static FederationServer createForDomain(String domain) {
    ...
    uriBuilder.append("/.well-known/HC-Net.toml");
    HttpUrl HcNetTomlUri = HttpUrl.parse(uriBuilder.toString());
    OkHttpClient httpClient = FederationServer.createHttpClient();

    Request request = new Request.Builder().get().url(HcNetTomlUri).build();
    Response response = null;
    try {
      response = httpClient.newCall(request).execute();

      if (response.code() >= 300) {
        throw new HcNetTomlNotFoundInvalidException();
      }

      Toml HcNetToml = new Toml().read(response.body().string());

      String federationServer = HcNetToml.getString("FEDERATION_SERVER");
  ...
  /**
   * Resolves a HC-Net address using a given federation server.
   * @param address HC-Net addres, like <code>bob*HC-Net.org</code>
